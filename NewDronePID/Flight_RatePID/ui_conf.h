...
BLYNK_WRITE(V2)
{
  roll_setpoint = param.asDouble(); // assigning incoming value from pin V1 to a variable
  ui_callback = 1;
}

BLYNK_WRITE(V3)
{
  pitch_setpoint = param.asDouble(); // assigning incoming value from pin V1 to a variable
  ui_callback = 1;
}

BLYNK_WRITE(V4)
{
  yaw_setpoint = param.asDouble(); // assigning incoming value from pin V1 to a variable
  ui_callback = 1;
}
...
BLYNK_CONNECTED()
{
  Blynk.virtualWrite(V0, eStop);
  Blynk.virtualWrite(V1, throttle);
  Blynk.virtualWrite(V2, roll_setpoint);
  Blynk.virtualWrite(V3, pitch_setpoint);
  Blynk.virtualWrite(V4, yaw_setpoint);
}*/
