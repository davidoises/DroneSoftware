...
double kp_pitch = 0; // 18.82
double kd_pitch = 0; // 1.559
double ki_pitch = 0; // 13.13
...
void loop() {
  ...
  if(update_orientation)
  {
    ...
    {
      // Roll PID
      double roll_error = roll_setpoint - orientation.get_roll()*180.0/PI;
      double roll_diff = (roll_error - roll_prev_error)/dt;
      roll_integral += roll_error*dt;
      roll_prev_error = roll_error;
      double roll_pid = kp*roll_error + ki*roll_integral + kd*roll_diff;

      // Pitch PID
      double pitch_error = pitch_setpoint - orientation.get_pitch()*180.0/PI;
      ...
      // Yaw PID
      double yaw_error = yaw_setpoint - orientation.get_yaw()*180.0/PI;
      double yaw_diff = (yaw_error - yaw_prev_error)/dt;
      yaw_integral += yaw_error*dt;
      yaw_prev_error = yaw_error;
      double yaw_pid = yaw_kp*yaw_error + yaw_ki*yaw_integral + yaw_kd*yaw_diff;
