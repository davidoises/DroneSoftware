...
double kp = 3.45;//4.2; // 3.7 // 0.11
double kd = 1.25;//1.45; // .015
double ki = 0.15;//0.04; // 0.02 // 0.015
...
void loop() {
  ...
  if(update_orientation_pid)
  {
    ...
    double integral = prev_integral + ki*(t_delta/2.0)*(error + prev_error);
    prev_integral = integral;
    prev_error = error;

    // PID calculation
    double torque_control_law = kp*error + integral + kd*diff;
    
    // Just use integral part when the error is small. This is just to reduce stationary error
    /*if(abs(error) < 3)
    {
      torque_control_law = kp*error + kd*diff;
    }*/
