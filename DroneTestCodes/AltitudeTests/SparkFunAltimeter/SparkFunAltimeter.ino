...
double inital_altitude = 0;
double altitude_memory[40] = {0};
double altitude_sum = 0;
uint8_t altitude_index = 0;
double altitude_samples = 40.0; //20
...
void setup()
{
  ...
  myPressure.setOversampleRate(2); // Set Oversample to the recommended 128 // 4 = 70ms, 3 = 40, 2=20
  myPressure.enableEventFlags(); // Enable all three pressure and temp event flags 

  delay(5000);
  for(int i = 0; i<20; i++)
  {
    inital_altitude += myPressure.readAltitude();  
    delay(20);
  }
  inital_altitude /= 20;
  ...
  {
    altitude_memory[i] = myPressure.readAltitude() - inital_altitude;
...
void loop()
{
  float current_altitude = myPressure.readAltitude() - inital_altitude;
  altitude_sum -= altitude_memory[altitude_index];
  altitude_memory[altitude_index] = current_altitude;
  altitude_sum += altitude_memory[altitude_index];
  altitude_index++;

  if(altitude_index == altitude_samples) altitude_index = 0;

  //altitude = 0.8*altitude + 0.9*altitude_sum/altitude_samples;
  
  
  Serial.print(current_altitude);
  Serial.print(" ");
  Serial.println(altitude_sum/altitude_samples);
