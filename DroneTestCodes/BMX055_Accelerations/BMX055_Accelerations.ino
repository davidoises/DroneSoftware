...
void setup() {
  ...
  {
    imu.get_acc_data();
    initial_acc_z += imu.accelerometer.z;
    delay(10);
  }
  initial_acc_z /= 100;
...
void loop() {
  if(update_orientation)
  {
    ...
    double roll = orientation.get_roll();
    double pitch = orientation.get_pitch();
    double az = (imu.accelerometer.z*cos(pitch)*cos(roll) - imu.accelerometer.x*sin(pitch) + imu.accelerometer.y*cos(pitch)*sin(roll)- initial_acc_z)*imu.accelerometer.res;
    pos += az*dt;
