...
void loop() {
  ...
  if(update_orientation)
  {
    ...
    double angle_error = pitch_setpoint - orientation.get_pitch()*180/PI;
    double pitch_rate_setpoint = kr*angle_error;
    
    double error = pitch_rate_setpoint - pitch_rate;
    double pitch_pid = 0;
    ...
    if(throttle >= 1100)
    {
      ...
      pitch_pid = kp*error + ki*integral + kd*diff;

      ma = constrain(throttle - pitch_pid/4.0, 1100, 2000); // Acts if negative torque is needed
      mb = constrain(throttle + pitch_pid/4.0, 1100, 2000); // Acts if negative torque is needed
      mc = constrain(throttle + pitch_pid/4.0, 1100, 2000); // Acts if positive torque is needed
      md = constrain(throttle - pitch_pid/4.0, 1100, 2000); // Acts if positive torque is needed
    }
    ...
    //Serial.print(error);
    //Serial.print(" ");
    Serial.println(orientation.get_pitch()*180/PI);
