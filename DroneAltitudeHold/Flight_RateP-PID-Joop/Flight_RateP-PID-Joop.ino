...
double kp_alt = 0;//1.4;//10
double ki_alt = 0;//0.2;
double kd_alt = 0;//0.75;//10
...
void loop() {
  ...
  if(update_orientation)
  {
    ...
    if(throttle >= 1100)
    {
      ...
      //Serial.println(alt_throttle_pid);
  ...
  if(update_altitude)
  {
    ...
    if(dt*1000.0 > 9.2)
    {
      ...
      pres_diff_av.memory[pres_diff_av.index] = (pressure*10 - prev_pressure);
      pres_diff_av.sum += pres_diff_av.memory[pres_diff_av.index];
      prev_pressure = pressure*10;
      pres_diff_av.index++;
      if(pres_diff_av.index == pres_diff_av.samples) pres_diff_av.index = 0;
      //pressure_diff = pres_diff_av.sum/pres_diff_av.samples;
      pressure_diff = pres_diff_av.sum;
      ...
      if(alt_hold)
      {
        ...
        double alt_error = alt_setpoint - pressure;
        ...
        alt_d = 0.7*alt_d + 0.3*kd_alt*pressure_diff;
        alt_throttle_pid = (kp_alt + kp_gain_altitude)*alt_error + ki_alt*alt_integral - alt_d;
